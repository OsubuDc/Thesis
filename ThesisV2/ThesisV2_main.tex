...
Normative ISO/TS 15066 covers the safety requirements for collaborative operations between human worker and robot. This normative classifies four different collaborative operations modes \cite{rosenstrauchHumanRobotCollaboration2018}\cite{chemwenoOrientingSafetyAssurance2020}: 
...
Hand--guiding
Hand--guided robotic applications are defined as a direct physical interaction between human and robot, where the human operator has full control over the robot motion. In this approach, a direct input device is located at or near the end--effector to measure input forces and simultaneously define the position of the human in the shared workspace. The robot must be in a monitored stop before hand--guiding can be activated via an enabling device. Safety requirements for this mode include force and or torque sensing at the guiding point, emergency stop accessible from the position where the robot is being guided, which is typically at the end--effector, and a maximum speed limit during guided motion.

Power and force limiting
This mode is the most commonly associated with collaborative robots. The robots' software and hardware are designed in such a way that any contact with a human cannot result in injury. Low robot mass, rounded surfaces and padding combined with active force and torque sensing, which limits contact forces with its environment below set thresholds depending on the body region of the human that can be in contact with the robot.

Speed and separation monitoring
Robot and human can operate in the same workspace at the same time, but the position and speed of the human and robot relative to each other is constantly monitored to adjust the speed of the robot with the goal of constantly having a minimum dynamical separation distance at all times. 

Hand--guiding is useful for packaging and assembly applications since it combines the physical strength of robotic arms, which can compensate the weight of components or help with moving relatively light but large components, with human intelligence.

ISO/TS 15066 specifies that the hand--guiding devices should be located at or near the robot end--effector. (not sure if its hand guiding or hand--guiding) 

A direct consequence of this approach is that robotic applications with sensorised end--effectors can only react in a passive compliant manner when handled at a specific point on the robotic arm. Collisions or attempted handling of the robotic arm elsewhere will result in a safety stop of the robotic arm \cite{sicilianoSpringerHandbookRobotics2016a}.

Existing robotic solutions that allow a human worker to hand--guide a robotic arm by grasping it wherever is most convenient are, to the best of the authors' knowledge, practically non--existent in assembly and packaging industries. When hand--guiding is discussed in literature, it is typically limited to sensorised end--effectors on conventional stiff robots. Collaborative robots that are adapted in industry are generally those without build in compliance. As stated before, collaborative robots are used for applications with little to no human contact where precision and repeatability is of more value than inherent compliance. \cite{gregorHandGuidingVirtual2021}\cite{hameedControlSystemDesign2023}.

This absence in current industrial applications is further supported by surveys of end users, system integrators and researchers, which indicate that collaborative robots are still primarily implemented as traditional stiff robots with limited or no direct human interaction but with the added value that less space is needed to operate these robots \cite{montiniCollaborativeRoboticsSurvey2024}.

Collaborative robots such as the UR30 from Universal Robots, and the CR-35iB from FANUC can handle payloads up to 30kg and 50kg respectively. Hand--guiding is mainly used for point based teaching, and path based teaching. The user can intuitively move the robot to a new point in space and record that position. This can be done for a singular point, or for a full path, in this case the robot records the full movement from start to finish while a button is pressed. It is however possible to correctly configure collaborative robots for weight compensation, but software is needed for correct calibration of the load attached to the end--effector. Although rare, some collaborative robots can estimate payload weight and center of mass by performing predefined controlled motions, but multiple interactions with the robot software controller are needed making this approach time consuming. This function however is not found often, and is often an optional software package and is not available for every collaborative robot, further proving that weight compensation of payloads of varying weights is mostly absent in industrial collaborative applications \cite{UR30CollaborativeRobot}\cite{CollaborativeRobotsCobots}\cite{groupHandGuidingRobots2021}\cite{ConfiguringFANUCRobot2026}. 

\subsection{Industrial manipulators for weight compensation}

Passive weight compensating manipulators are well established in industrial environments. By combining continuous force springs like coil springs or gas springs which can sometimes be adjusted, with load bearing joints, humans only have to provide a small force to move heavy objects. These are, as stated before, passive and can thus not be used for autonomous tasks. They are also pre-tuned for specific weights, and can only be used in a specific weight range. Pressurized air needs more maintenance then electric solutions and is also more expensive. I LL ADD TO THIS LATER
